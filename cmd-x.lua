...
-- Constraints: -----------------------|
local ver = "1.9.9e"
local cordCode = "https://discord.gg/ySENcKM8zv"
---------------------------------------|
...
Stand("atrium V2 is in development.", "Please join the discord for development updates.", "You can get the discord invite by using the .discord command.", "", "", "Continue", true)

wait(.5)

loadstring(game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/Version",true))()
if Current and Current.Version ~= ver then
	Stand("atrium is not up to par!","atrium cannot run.","","Your version: "..ver,"Current version: "..Current.Version,"Run Anyway",true)
...
function updatesaves()
	...
	writefile("atrium.lua", game:GetService("HttpService"):JSONEncode(update))
end

function loadsaves()
	local success, errorsend = pcall(function()
		saves = game:GetService("HttpService"):JSONDecode(readfile("atrium.lua"))
	end)
	if not success then
		loaddefaults()
		updatesaves()
...
if AntiCheat.HideParentInExploit then
	...
	elseif get_hidden_gui then
		...
		Stand("Your exploit does not support syn.protect_gui/get_hidden_gui!","atrium cannot run.","Explanation;","This game has an anti-cheat and our way of stopping it is through syn.protect_gui/get_hidden_gui.","","Run Anyway",true)
		repeat wait() until RunDude == true
	end
end
...
if AntiCheat.RemoveScripts then
	...
	Stand("atrium has to delete local scripts!","Your game may not work.","Explanation;","This game has an anticheat and our way of solving it is removing scripts.","","Run Anyway",true)
	repeat wait() until RunDude == true
end
...
local Hints = {"Did you know you can execute multiple commands with ';;'?","Did you know you can create a wait using timedcmd in loops?","You can find commands using 'cmds'.","You can change your prefix by doing 'prefixnew (key)'","Did you know atrium was created in October 2019?"}
...
opxviewertitle.Text = "atrium OUTPUT-LONGER"
...
statviewertitle.Text = "atrium STATISTICS VIEWER"
...
Save.MouseButton1Down:Connect(function()
	...
	writefile("atrium Stats "..math.random(1000)..".txt",tostring(writestats))
end)
...
logsviewertitle.Text = "atrium LOGS VIEWER"
...
Save2.MouseButton1Down:Connect(function()
	...
	writefile("atrium Logs "..math.random(1000)..".txt",tostring(writestats2))
end)
...
CMDS.commands = {
	...
	["entercmdnew"] = "Adds a new command which runs when you first execute atrium.",
	...
	["support"] = "If you need support while using atrium use this command!",
	["commands"] = "Shows this page.",
	["advertise"] = "Help spread atrium by doing this cmd in game! (<3)",
	["unadvertise"] = "()",
	...
	["credits"] = "Lists credits for atrium.",
	...
	["streamermode"] = "Hides all names in games (Suitable for streamers or people showcasing atrium <3).",
	...
	["changestyle"] = "Changes the style of your atrium refer to csinfo for more info on this command.",
	...
	["using"] = "Specifies that you are using atrium. <3",
	...
	["runafter"] = "Makes atrium automatically re-execute on rejoin.",
	...
	["unbackpackplay"] = "()",
	["migratesaves"] = "Migrates save files/backups to atrium.lua.",
	...
	["rejoinexecute"] = "Rejoins and re-executes atrium once.",
	...
	["nilchatcmds"] = "Makes atrium chat commands not appear on chat.",
	["unnilchatcmds"] = "()",
	...
	["unretardchat"] = "()",
	["scriptusers"] = "Shows people currently using atrium.",
	...
	["yesmessages"] = "()",
	["removecmd"] = "Removes command from your atrium temporarily.",
	...
	["discord"] = "Sets your clipboard to a atrium discord invite."
}
CMDS.usage = {
	["promptnew"] = "(str/name)",
	["prefixnew"] = "(str)",
...
fileext.Text = "plugins go inside atrium Plugins folder located in workspace"
...
function opx(f,text)
	...
	if not ChatBind then
		...
		if f == "*" then
			sayremote:FireServer("[atrium*] "..text,"All")
		elseif f == "-" then
			sayremote:FireServer("[atrium-] "..text,"All")
		end
	end
...
ManageTitleEnter.Text = "atrium EnterCMDs MANAGER"
...
function useCommand.advertise()
	...
	opx("-","Now advertising atrium (Thank you!)")
end

function useCommand.unadvertise()
	cmdAdvertise = false
	opx("-","Stopped advertising atrium (Thank you!)")
end

function useCommand.version()
	opx("-","atrium is currently on "..ver)
end
...
function useCommand.using()
	opx("-","Said script <3")
	sayremote:FireServer("I am using atrium, Version: "..ver, "All")
end
...
function useCommand.playerstalker()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/projetcs/PlayerStalkerV2",true))()
	opx("-","Loaded PlayerStalker")
end

function useCommand.badger()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/badger",true))()
...
function useCommand.resetsaves()
	...
	if Confirmation then
		opx("-","Successfully reset your atrium.")
		if isfile("atrium.lua") then
			delfile("atrium.lua")
		end
	end
end
...
function useCommand.savesaves()
	...
	setclipboard(readfile("atrium.lua"))
	writefile("atrium.save",readfile("atrium.lua"))
end
	...
	if Confirmation then
		if arguments[2] then
			if isfile(arguments[2]) then
				...
				writefile("atrium.lua",readfile(arguments[2]))
			else
				opx("*","You do not have a save by that name.")
			end
		else
			if isfile("atrium.save") then
				opx("-","Migrated atrium.save to current saves")
				writefile("atrium.lua",readfile("atrium.save"))
			else
				opx("*","You do not have a atrium save.")
			end
		end
	end
end
...
function useCommand.messagebox()
	if messageboxasync then
		...
		messageboxasync(getstring(2), "atrium Message", 262148)
	else
...
function useCommand.runafter()
	...
	elseif arguments[2] == "off" then
		opx("-","atrium will no longer run after rejoin")
		KeepCMDXOn = false
		updatesaves()
	else
		opx("*","2 arguments are required!")
...
function useCommand.audiologgersave()
	...
	writefile(filename..".atrium", audioIds)
end
...
function useCommand.singinfo()
	...
	To see a sample of what to do, visit: https://raw.githubusercontent.com/atrium/atrium/master/others/sing/")
end
...
function useCommand.sing()
	...
	if check[string.lower(arguments[2])] then
		getSong = "https://raw.githubusercontent.com/atrium/atrium/master/others/sing/"..string.lower(arguments[2])
	else
		getSong = arguments[2]
	end
...
function useCommand.antiwrldsgui()
	...
	loadstring(game:HttpGet(('https://raw.githubusercontent.com/atrium/atrium/master/others/antiwrldsgui/main'),true))()
...
function useCommand.retardchat()
	...
	local stringka = game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/unicode"):split(",")
...
function useCommand.admin()
	if arguments[2] then
		...
		if target then
			...
			target.Chatted:Connect(function(a)
				local comsToExe = a:split(";;")
				for i,v in pairs(comsToExe) do
					if v ~= BlacklistedADM[v] then
						executeCommand(v)
			...
			updatesaves()
			opx("-","Added "..target.Name.." to admins.")
		else
			opx("*","Player does not exist!")
		end
	else
...
ManageTitleHotkey.Text = "atrium Hotkey MANAGER"
...
function useCommand.clearchat()
	opx("-","Cleared chat")
	for i = 1,3 do
		sayremote:FireServer(string.rep("ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¸ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â»", 200), "All")
	end
end
...
function useCommand.scriptusers()
	if sDetect then
		...
		opx("-","Now showing users using atrium")
...
function useCommand.playercases()
	...
	using - Chooses a player that is using atrium")
end
...
function useCommand.randomchat()
	local tbl1 = game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/randomchat/1"):gsub('"',""):gsub('{',""):gsub('}',""):split(",")--english words
	local tbl2 = game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/randomchat/2"):gsub('"',""):gsub('{',""):gsub('}',""):split(",")--tbl1
	local tbl3 = game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/randomchat/3"):gsub('"',""):gsub('{',""):gsub('}',""):split(",")--tbl2
	local tbl4 = game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/randomchat/4"):gsub('"',""):gsub('{',""):gsub('}',""):split(",")--tbl3
	local tbl5 = game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/randomchat/5"):gsub('"',""):gsub('{',""):gsub('}',""):split(",")--tbl5
	local tbl6 = game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/randomchat/6"):gsub('"',""):gsub('{',""):gsub('}',""):split(",")--tbl4
	local tbl7 = game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/randomchat/7"):gsub('"',""):gsub('{',""):gsub('}',""):split(",")--tbl6
	local tbl8 = game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/randomchat/8"):gsub('"',""):gsub('{',""):gsub('}',""):split(",")--tbl7
	local tbl9 = game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/randomchat/9"):gsub('"',""):gsub('{',""):gsub('}',""):split(",")--tbl8
...
function useCommand.credits()
	...
	Thank you to IY for some functions inside atrium\
...
pcall(function() loadstring(game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/depthoffield",true))() end)
...
function useCommand.spotify()
	...
	loadstring(game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/projetcs/spotifypresence"))()
end
...
function useCommand.sentinelexplorer()
	...
	loadstring(game:HttpGet("https://raw.githubusercontent.com/atrium/atrium/master/others/sdex", true))()
...
function useCommand.logchat()
	...
	if arguments[2] then
		...
		msg = string.rep("atrium Gang", 100)
	end
...
if not AntiCheat.PrintingOff then
	...
	StarterGui:SetCore("ChatMakeSystemMessage", {
		Text = "Welcome to atrium, "..cmdlp.Name..".";
...
for _,v in pairs(cmdp:GetPlayers()) do
	if Devs[v.Name] then
		opx("-",v.Name.." is in your server ("..Devs[v.Name].." of atrium)")
		RGBDev(v)
